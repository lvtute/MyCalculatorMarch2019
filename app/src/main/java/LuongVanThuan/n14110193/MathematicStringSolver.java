...
public class MathematicStringSolver {
    boolean check_error = false;  // check the first character is positive or negative
    // , check error

    public String standardizeDouble(double num){
        int a = (int)num;
        if (a == num)
            return Integer.toString(a);//use this to eliminate the decimal part
    ...
    public String[] processString(String sMath){ //
        ...
        MathematicStringSolver  MSS = new MathematicStringSolver();
        for (int i=0; i<sMath.length(); i++){
            char c = sMath.charAt(i);
            if (i<sMath.length()-1 && isCharPi(c) && !MSS.isOperator(sMath.charAt(i+1))){ // error neu co dang Ï€3
                check_error = true;
                return null;
            }
            else
    ...
    public String valueMath(String[] elementMath){
        ...
        for (int i=0; i<elementMath.length; i++){
            ...
            if (isCharPi(c)) S.push(Math.PI);	// neu la pi
                ...
                if (!MSS.isOperator(c)) S.push(Double.parseDouble(elementMath[i])); //so
                else{	// toan tu
